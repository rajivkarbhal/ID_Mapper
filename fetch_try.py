...
if (sep := re.match(r".+\,.+", str(gen), re.IGNORECASE)):
	...
	for ecg in gnl:
		concstring = ""
		url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gene&term=%s[Gene Name]&retmax=100000"%ecg
		response = requests.get(url)
		blocks = response.text.split("\n")
		resulting_string = ' '.join(blocks)
		if (itct := re.match(r".+\<Count\>0.+", str(resulting_string), re.IGNORECASE)):
			print (ecg,": Please check gene symbol, data not available")
		else:
			for gnidc in enumerate(blocks):
				if (pnm := re.match(r".+\<Id\>(\d+)\<\/Id\>.+", str(gnidc), re.IGNORECASE)):
					bul = pnm.group(1)
	...
	print ("Single gene:", gen)
	concstring = ""
	url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gene&term=%s[Gene Name]&retmax=100000"%gen
	response = requests.get(url)
	blocks = response.text.split("\n")
	resulting_string = ' '.join(blocks)
	if (itct := re.match(r".+\<Count\>0.+", str(resulting_string), re.IGNORECASE)):
		print (gene,": Please check gene symbol, data not available")
	else:
		for gnidc in enumerate(blocks):
			if (pnm := re.match(r".+\<Id\>(\d+)\<\/Id\>.+", str(gnidc), re.IGNORECASE)):
				bul = pnm.group(1)
...
#url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gene&term=Brca2[Gene Name]&retmax=100000"
...
if (sep := re.match(r".+\,.+", str(gen), re.IGNORECASE)):
	...
	for ecg in gnl:
		...
		if (itct := re.match(r".+\<Count\>0.+", str(resulting_string), re.IGNORECASE)):
			...
			#print (gid)
			for resline in (gid):
				splres = resline.split('_')
				ids = splres[0]
				ogs = splres[1]
				smb = splres[2]
				if smb == ecg:
					concstring += str(resline) + "/"
					#gacid.append(ids)
		#print (concstring,"\n")
		gacid.append(concstring)
		blocks.clear()
else:
	print ("Single gene:", gen)
	concstring = ""
